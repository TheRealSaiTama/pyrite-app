...
export default function HoverReceiver() {
  const [hoverBox, setHoverBox] = React.useState<Box>(null);
  const [hoverBoxes, setHoverBoxes] = React.useState<Box[]>([]);
  const [focusBox, setFocusBox] = React.useState<Box>(null);
  const [focusedElementId, setFocusedElementId] = React.useState<string | null>(null);
  const [isVisualEditMode, setIsVisualEditMode] = React.useState(() => {
  ...
  const [focusTag, setFocusTag] = React.useState<string | null>(null);
  const isResizingRef = React.useRef(false);
  const lastHitElementRef = React.useRef<HTMLElement | null>(null);
  const lastHitIdRef = React.useRef<string | null>(null);
  const focusedElementRef = React.useRef<HTMLElement | null>(null);
  const isVisualEditModeRef = React.useRef(false);
  const scrollTimeoutRef = React.useRef<number | null>(null);
  const originalContentRef = React.useRef<string>("");
  const originalSrcRef = React.useRef<string>("");
  const focusedImageElementRef = React.useRef<HTMLImageElement | null>(null);
  const editingElementRef = React.useRef<HTMLElement | null>(null);
  const wasEditableRef = React.useRef<boolean>(false);
  const styleElementRef = React.useRef<HTMLStyleElement | null>(null);
  const originalStylesRef = React.useRef<Record<string, string>>({});
  const appliedStylesRef = React.useRef<Map<string, Record<string, string>>>(
    new Map()
  );
  const hasStyleChangesRef = React.useRef<boolean>(false);
  const lastClickTimeRef = React.useRef<number>(0);
  const pendingCleanupRef = React.useRef<NodeJS.Timeout | null>(null);

  const loadedFontFamilies = React.useRef<Set<string>>(new Set());
  const persistentFontMap = React.useRef<Map<string, string>>(new Map());
  const persistentFontTimeouts = React.useRef<Map<string, number>>(new Map());
